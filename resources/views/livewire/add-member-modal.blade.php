<div>
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" wire:click="$set('isOpen', false)"></div>
        ...
        <div class="relative w-full max-w-xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-slate-900">Tambah Anggota</h3>
                        <p class="text-[11px] text-slate-500 font-medium uppercase tracking-wider">Relasi: <span class="text-indigo-600 capitalize font-bold">{{ $relationshipType }}</span></p>
                    </div>
                </div>
                <button wire:click="$set('isOpen', false)" class="text-slate-400 hover:text-slate-600 transition p-2 hover:bg-slate-50 rounded-lg">
                ...
                <form wire:submit.prevent="save" class="space-y-6">
                    ...
                    <div x-data="{ photoPreview: null }" class="flex flex-col items-center justify-center space-y-3 mb-2">
                        <div class="relative group">
                            <div class="w-24 h-24 rounded-full border-4 border-white shadow-md overflow-hidden bg-slate-100 flex items-center justify-center">
                                ...
                                <!-- Fallback to Placeholder (Server Side) -->
                        ...
                        <span class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Foto Profil</span>
                    </div>

                    @if($relationshipType === 'child' && count($spouses) > 0)
                    <div class="space-y-1.5 p-4 bg-indigo-50/50 rounded-xl border border-indigo-100 mb-6">
                        <label class="block text-sm font-bold text-indigo-900">Pilih Orang Tua Lainnya (Opsional)</label>
                        <p class="text-[11px] text-indigo-600 mb-2">Agar anak terhubung ke pasangan yang benar dan tidak duplikat</p>
                        <div class="relative group">
                            ...
                            <select wire:model="otherParentId" class="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl text-sm text-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition outline-none shadow-sm bg-white cursor-pointer">
                                <option value="">Tanpa Pasangan (Orang Tua Tunggal)</option>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <div class="relative group">
                                ...
                                <input wire:model="firstName" type="text" placeholder="Supena" class="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl text-sm text-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition outline-none shadow-sm">
                            </div>
                            @error('firstName') <span class="text-[11px] text-red-500 font-medium">{{ $message }}</span> @enderror
                        </div>
                        <div class="space-y-1.5">
                            ...
                            <div class="relative group">
                                ...
                                <input wire:model="lastName" type="text" placeholder="Asli" class="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl text-sm text-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition outline-none shadow-sm">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <button type="button" wire:click="$set('gender', 'M')" class="flex items-center gap-3 p-3 rounded-xl border-2 transition text-sm font-bold {{ $gender === 'M' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 shadow-sm' : 'bg-white border-slate-100 text-slate-500 hover:border-slate-200 hover:bg-slate-50' }}">
                    ...
                    <div class="space-y-1.5 pb-2">
                        ...
                        <div class="relative group">
                            ...
                            <textarea wire:model="bio" rows="3" placeholder="Ceritakan sedikit tentang beliau..." class="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl text-sm text-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition outline-none resize-none shadow-sm"></textarea>
                        </div>
                    </div>

                    <!-- Footer - Action Buttons -->
                    <div class="flex gap-3 pt-6 sticky bottom-0 bg-white pb-2 mt-4 space-y-0">
                        <button type="button" wire:click="$set('isOpen', false)" class="flex-1 py-3.5 bg-slate-50 hover:bg-slate-100 text-slate-600 font-bold rounded-xl transition text-sm">Batal</button>
